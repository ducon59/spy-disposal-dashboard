...
*Price as of: {spy_ts}*
    ...
    spy_cost_usd = st.slider(
        ...
        value=555_000, step=5_000, format="$%d",
    ...
    other_capital_gains_usd = st.slider(
        "Other UK based latent gain ($)",
        min_value=0, max_value=2_000_000,
        value=400_000, step=25_000, format="$%d",
        help="UK-based capital gain that would only be crystallised under the flat tax regime. Not applicable in the baseline (Italian IRPEF) or relocation scenarios — it is assumed this gain is held and not disposed of in those cases."
    )
    ...
    portfolio_growth_rate = st.slider(
        ...
        min_value=0, max_value=20, value=7, step=1, format="%d%%",
...
# Other UK based latent gain:
# - Baseline ①: NOT included — this gain is only crystallised if the flat tax regime is elected
# - Flat Tax ②: lump sum covers it in Year 1 — no extra CGT beyond the €300k lump sum
# - Flat Tax ③: lump sum covers it across regime years — no extra CGT
# - Relocation ④: NOT included — assumed not crystallised during relocation (held in UK wrapper)
# The gain is only relevant as an additional benefit of the flat tax option.
uk_gain_flat_tax_eur    = other_cap_gains_eur  # full gain sheltered by lump sum — no CGT cost
other_cgt_baseline_eur  = 0.0   # not crystallised in baseline
other_cgt_reloc_eur     = 0.0   # not crystallised during relocation
...
with st.expander("📐 IRPEF & Baseline Calculation Detail (EUR)"):
    ...
    | UK latent gain (not crystallised in baseline) | €0 |
...
card(c1, "①", f"Sell {holding_label} — IRPEF",
     ...
     f"IVAFE: €{ivafe_total_eur:,.0f}. UK latent gain: not crystallised in this scenario. "
...
card(c2, "②", "Flat Tax — 1 Year",
     ...
     f"UK latent gain of €{uk_gain_flat_tax_eur:,.0f} also sheltered by lump sum at no extra cost. Total: €{s2_total_eur:,.0f}. "
...
card(c3, "③", f"Flat Tax — {multi_yrs} Years",
     ...
     f"Total lump sum: €{s3_lump_eur:,.0f}. UK latent gain of €{uk_gain_flat_tax_eur:,.0f} sheltered by regime — €0 extra CGT. "
...
card(c4, "④", f"Relocate ({int(reloc_rate*100)}% CGT)",
     ...
     f"UK latent gain not crystallised — assumed held. Total: €{s4_total_eur:,.0f}. "
...
df = pd.DataFrame([
    {"Scenario": f"① Sell {holding_label} (IRPEF)",
     ...
     "IVAFE (€)": f"€{ivafe_total_eur:,.0f}", "Other CGT (€)": "€0 (UK gain not crystallised)",
    ...
    {"Scenario": "② Flat Tax 1yr (€300k)",
     ...
     "IVAFE (€)": "€0", "Other CGT (€)": f"€0 (UK gain €{uk_gain_flat_tax_eur:,.0f} sheltered)",
    ...
    {"Scenario": f"④ Relocate {int(reloc_rate*100)}%",
     ...
     "IVAFE (€)": f"€{reloc_running_eur:,.0f}", "Other CGT (€)": "€0 (UK gain not crystallised)",
...
**UK latent gain:** €{other_cap_gains_eur:,.0f} — **not crystallised in this scenario**. This gain is only assumed to be realised if electing the flat tax regime, where it is sheltered by the lump sum.
...
- UK latent gain (€{other_cap_gains_eur:,.0f}) ✓ — sheltered by the lump sum as a foreign-source gain → **€0 extra CGT**

**Total cost:** €300,000 lump sum = **€{s2_total_eur:,.0f}** · UK latent gain of €{other_cap_gains_eur:,.0f} also sheltered at no extra cost · **Saved: €{s2_saved_eur:,.0f}**
...
- **{extra_years_line}**
- **Total flat tax:** €{s3_lump_eur:,.0f} · **UK latent gain €{other_cap_gains_eur:,.0f}: sheltered by regime (€0 extra CGT)**
...
The UK latent gain of €{other_cap_gains_eur:,.0f} is sheltered by the regime — no CGT applies to foreign-source gains while the lump sum is in force. This gain would not otherwise be crystallised.
    """)
...
| UK latent gain (not crystallised — held) | €0 |
